...
(async()=>{
  ...
  try {
    for(const theme of ['light','dark']) {
      ...
      await card.getByText('Basis vor AC-Begrenzung',{exact:true}).waitFor();
      ...
      await card.locator('#explanation-toggle').click();
      ...
      assert.deepEqual(errors,[]);await page.close();console.log(`360 ${theme}: Erklärung, AC-Bilanz, Dach-/Tageswechsel, Touch und Tastatur bestanden.`);
    }
