...
function renderChart(state, width, selectedKey) {
  ...
  return `<svg id="interval-chart" class="chart" viewBox="0 0 ${width} 242" tabindex="0" role="group" aria-roledescription="Interaktives Diagramm" aria-labelledby="chart-title" aria-describedby="chart-description chart-help">
    <title id="chart-title">Energie je Intervall in kWh</title><desc id="chart-description">${view.historical ? "Archivierte Prognose: " + escapeHtml(view.label) : "Aktuelle Prognose durchgezogen, " + ARCHIVE_LABEL} gestrichelt, Tatsächliche Produktion als farbige Stufenlinie mit dezenten Balken; teilweise erfasste Mengen gestrichelt, vollständige Menge unbekannt.${series.raw ? " Grundmodell ohne Selbstkalibrierung strichpunktiert." : ""} Fehlende Werte bleiben leer. Alle Werte stehen auch in der Tabelle.</desc>
...
export function renderContent(config, state, width = 600, report = null, reportDays = 7, planningUI = {}, selectedKey = null, archiveUI = {}) {
  ...
  return `<div class="header"><div><p class="eyebrow">PV FORECAST</p><h2 tabindex="-1">${escapeHtml(title)}</h2><p class="subtitle">${escapeHtml(scope)} · ${escapeHtml(formatPlantDate(view.start, view.timezone))}</p></div><span class="badge ${view.stale ? "warning" : ""}">${forecast.envelope?.origin === "restored" ? "Gespeicherte Prognose" : forecast.retained ? "Letzter Stand" : view.stale ? "Veraltet" : "Prognose"}</span></div>
    ...
    <section class="chart-section" aria-label="Tagesverlauf"><div class="chart-heading"><h3>Energie im Tagesverlauf · ${view.day === "tomorrow" ? "Morgen" : "Heute"}</h3><span>kWh / Intervall</span></div><div class="legend"><span><i class="forecast-key"></i>Aktuelle Prognose</span><span><i class="history-key"></i>${ARCHIVE_LABEL}</span><span><i class="actual-key"></i>${hasDerivedEnergy(state) ? "Ertrag · enthält berechnete Energie" : "Tatsächlich produziert"}</span>${tableRows(state).some((row) => finite(row.actual_observed)) ? '<span><i class="actual-key actual-partial-key"></i>Teilweise erfasst</span>' : ""}${state.showRaw && !view.roof_id && currentExplanation(state)?.status === "available" && !view.roof_id ? '<span><i class="raw-key"></i>Grundmodell ohne Selbstkalibrierung</span>' : ""}</div>${renderChart(state, width, selectedKey)}${tableRows(state).some((row) => finite(row.actual_observed)) ? '<p class="hint">Gestrichelt: nur belegte Teilmengen der Stunde. Die vollständige Produktion ist unbekannt; Mengen über Stundengrenzen werden nicht aufgeteilt.</p>' : ""}${renderIntervalDetails(state, selectedKey)}<p class="chart-note">${escapeHtml(view.timezone)} · Ansicht ${escapeHtml(formatPlantTime(view.as_of, view.timezone))}<br>Wetterabruf ${escapeHtml(weatherStamp)}${forecast.envelope?.origin === "restored" ? "<br>Gespeicherter Stand · Aktualisierung fehlgeschlagen" : ""}${forecast.retained || measurement?.retained ? `<br>Letzte gelesene Ansicht: ${escapeHtml(plantStamp(view.as_of, view.timezone))}. Messfenster bis ${escapeHtml(plantStamp(measurement?.data?.end ?? measurement?.data?.outlook?.as_of ?? view.as_of, view.timezone))}.` : ""}</p></section>
