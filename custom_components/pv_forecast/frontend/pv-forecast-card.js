...
export function renderIntervalDetails(state, key) {
  ...
  return `<section id="interval-detail" data-start="${escapeHtml(detail.start)}" data-end="${escapeHtml(detail.end)}" class="interval-detail" aria-label="Ausgewähltes Intervall"><h3>${escapeHtml(plantStamp(detail.start, view.timezone))}<br>bis ${escapeHtml(plantStamp(detail.end, view.timezone))}</h3><p class="hint">${escapeHtml(view.timezone)} · kWh je tatsächlichem Intervall</p><dl class="interval-values">${Object.entries({ ...(view.historical ? {} : { forecast: "Aktuelle Prognose", ...(detail.sources.raw ? { raw: "Grundmodell ohne Selbstkalibrierung" } : {}) }), history: view.historical ? view.label : ARCHIVE_LABEL, actual: "Messung" }).map(([name, label]) => {
    ...
    return `<div><dt>${label}</dt><dd>${source.status === "complete" ? `${energyText(source.energy_kwh)} kWh · vollständig` : source.status === "incomplete" ? finite(source.observed_energy_kwh) ? `${energyText(source.observed_energy_kwh)} kWh · unvollständig erfasst; vollständige Menge unbekannt` : "— · unvollständig" : "— · nicht vorhanden"}${source.quality_flags.includes("derived_energy") ? '<span class="hint">Enthält aus Leistung berechnete Energie</span>' : ""}${source.quality_flags.some((flag) => flag !== "derived_energy") ? '<span class="hint">Qualitätsmarkierungen vorhanden</span>' : ""}</dd></div>`;
...
function renderChart(state, width, selectedKey) {
  const view = state.forecast.data;
  const series = { ...selectedSeries(state) };
  // Archivkurven gehören in die Archivansicht, nicht zu den aktuellen Messreihen.
  if (!view.historical) series.history = [];
  const { left, top, bottom, x } = plotGeometry(view, width);
  const partialActual = series.actual.map((item) => ({ ...item, energy_kwh: item.energy_complete !== true ? item.observed_energy_kwh : null }));
  const values = [...partialActual.map((item) => item.energy_kwh).filter((value) => finite(value) && value >= 0), ...Object.values(series).flat().filter((item) => item.is_complete !== false && item.energy_complete !== false).map((item) => item.energy_kwh).filter(finite)];
  ...
  const actual = series.actual.filter((item) => item.energy_complete === true && finite(item.energy_kwh)).map((item) => {
    ...
    return `<rect class="actual-bar" x="${x(item.start) + 2}" y="${bottom - height}" width="${barWidth}" height="${height}"><title>${escapeHtml(formatPlantTime(item.start, view.timezone))}: ${energyText(item.energy_kwh)} kWh ${item.quality_flags?.includes("derived_energy") ? "mit aus Leistung berechnetem Anteil" : "gemessen"}</title></rect>`;
  }).join("");
  ...
  return `<svg id="interval-chart" class="chart" viewBox="0 0 ${width} 242" tabindex="0" role="group" aria-roledescription="Interaktives Diagramm" aria-labelledby="chart-title" aria-describedby="chart-description chart-help">
    <title id="chart-title">Energie je Intervall in kWh</title><desc id="chart-description">${view.historical ? "Archivierte Prognose: " + escapeHtml(view.label) + " gestrichelt" : "Aktuelle Prognose durchgezogen"}, Tatsächliche Produktion als farbige Stufenlinie mit dezenten Balken; unvollständig erfasste Mengen gestrichelt, vollständige Menge unbekannt.${series.raw ? " Grundmodell ohne Selbstkalibrierung strichpunktiert." : ""} Fehlende Werte bleiben leer. Alle Werte stehen auch in der Tabelle.</desc>
    <defs><clipPath id="plot-clip"><rect x="${left}" y="0" width="${width - left - 12}" height="${bottom + 2}"/></clipPath></defs>
    ${grid}<g clip-path="url(#plot-clip)">${gaps}${highlight}${actual}${paths(series.history, "history-line")}${paths(series.raw ?? [], "raw-line", "is_complete")}${paths(series.forecast, "forecast-line", "is_complete")}${paths(series.actual.map((item) => ({ ...item, energy_complete: item.energy_complete === true })), "actual-line", "energy_complete")}${paths(partialActual, "actual-line actual-partial-line")}${now}</g><g aria-hidden="true" class="hour-ticks">${hourTicks}</g>${ticks}
...
/** Herkunft derselben Gesamtmessung verwenden wie die Ist-Kennzahl. */
function hasDerivedEnergy(state) {
  if (state?.forecast?.data?.roof_id) return false;
  const data = state?.measurement?.data;
  const total = data?.current_location_total_energy ?? data?.total_energy;
  return total?.quality_flags?.includes("derived_energy") === true;
}
...
export function dataNotices(state) {
  ...
  if (view) {
    ...
    if (view.roof_id) add("roof", "Information", "Nur Dachprognose", "Keine Dachmessung zugeordnet. Das Archiv bezieht sich auf die Gesamtanlage.", "Für Messung und Archiv die Gesamtanlage auswählen.");
      ...
      else if (!total?.energy_complete) add("measurement-incomplete", "Einschränkung", finite(total?.energy_kwh) ? "Messung unvollständig" : "Noch keine Messung", finite(total?.energy_kwh) ? "Ist heute ist nur der bisher belegte Teil. Fehlende Erfassung ist kein Nullertrag." : "Für heute sind noch keine belegten Messwerte verfügbar.", "Messquelle prüfen und weitere Zählerstände abwarten. Ein fehlender Tagesbeginn wird nicht nachträglich erfunden.");
    }
    if (hasDerivedEnergy(state)) add("derived-energy", "Information", "Ertrag enthält berechnete Energie", "Mindestens eine Energiequelle wird aus gemeldeten Leistungswerten angenähert, etwa beim KOSTAL-KSEM-Assistenten. Auch eine vollständige Erfassung ist damit kein exakter Energiezählerstand.", "Beim Portalvergleich denselben Tag, Endzeitpunkt und AC-Messumfang verwenden. Seltene Meldungen und schnelle Leistungswechsel können Abweichungen verursachen. Wenn vorhanden, einen echten AC-Ertragszähler zuordnen; Wettermodell und Kalibrierung ändern Ist heute nicht.");
    ...
    const flags = view.intervals.some((item) => item.quality_flags?.length) || state.measurement?.data?.total_energy?.quality_flags?.some((flag) => flag !== "derived_energy") || selectedSeries(state).history.some((item) => item.quality_flags?.length);
...
export function renderContent(config, state, width = 600, report = null, reportDays = 7, planningUI = {}, selectedKey = null, archiveUI = {}) {
  ...
  return `<div class="header"><div><p class="eyebrow">PV FORECAST</p><h2 tabindex="-1">${escapeHtml(title)}</h2><p class="subtitle">${escapeHtml(scope)} · ${escapeHtml(formatPlantDate(view.start, view.timezone))}</p></div><span class="badge ${view.stale ? "warning" : ""}">${forecast.envelope?.origin === "restored" ? "Gespeicherte Prognose" : forecast.retained ? "Letzter Stand" : view.stale ? "Veraltet" : "Prognose"}</span></div>
    ...
    <section class="chart-section" aria-label="Tagesverlauf"><div class="chart-heading"><h3>Energie im Tagesverlauf · ${view.day === "tomorrow" ? "Morgen" : "Heute"}</h3><span>kWh / Intervall</span></div><div class="legend"><span><i class="forecast-key"></i>Aktuelle Prognose</span><span><i class="actual-key"></i>${hasDerivedEnergy(state) ? "Ertrag · enthält berechnete Energie" : "Tatsächlich produziert"}</span>${tableRows(state).some((row) => finite(row.actual_observed)) ? '<span><i class="actual-key actual-partial-key"></i>Unvollständig erfasst</span>' : ""}${state.showRaw && !view.roof_id && currentExplanation(state)?.status === "available" && !view.roof_id ? '<span><i class="raw-key"></i>Grundmodell ohne Selbstkalibrierung</span>' : ""}</div>${renderChart(state, width, selectedKey)}${tableRows(state).some((row) => finite(row.actual_observed)) ? '<p class="hint">Unvollständig erfasst (gestrichelt): nur belegte Teilmengen der Stunde. Die vollständige Produktion ist unbekannt; Mengen über Stundengrenzen werden nicht aufgeteilt.</p>' : ""}${renderIntervalDetails(state, selectedKey)}<p class="chart-note">${escapeHtml(view.timezone)} · Ansicht ${escapeHtml(formatPlantTime(view.as_of, view.timezone))}<br>Wetterabruf ${escapeHtml(weatherStamp)}${forecast.envelope?.origin === "restored" ? "<br>Gespeicherter Stand · Aktualisierung fehlgeschlagen" : ""}${forecast.retained || measurement?.retained ? `<br>Letzte gelesene Ansicht: ${escapeHtml(plantStamp(view.as_of, view.timezone))}. Messfenster bis ${escapeHtml(plantStamp(measurement?.data?.end ?? measurement?.data?.outlook?.as_of ?? view.as_of, view.timezone))}.` : ""}</p></section>
...
const styles = `
  ...
  *{box-sizing:border-box}ha-card{display:block;background:var(--ha-card-background,var(--card-background-color,#fff));border-radius:var(--ha-card-border-radius,16px);border:var(--ha-card-border-width,1px) solid var(--ha-card-border-color,var(--divider-color,#e4e8eb));box-shadow:var(--ha-card-box-shadow,none);overflow:hidden} .body{padding:22px 22px 8px;min-width:0}
  ...
  .chart-heading{display:flex;justify-content:space-between;align-items:baseline;gap:8px}h3{font-size:var(--pv-text);font-weight:600;margin:0}.chart-heading>span{font-size:var(--pv-text);color:var(--pv-muted)}.legend{display:flex;flex-wrap:wrap;gap:8px 16px;margin:12px 0 8px;color:var(--pv-muted);font-size:var(--pv-text);line-height:1.4}.legend span{display:inline-flex;align-items:center;gap:6px}.legend i{display:inline-block;width:18px;flex-shrink:0}.forecast-key{border-top:3px solid var(--pv-line)}.history-key{border-top:2px dashed var(--pv-archive)}.actual-key{border-top:3px solid var(--pv-actual)}.chart{display:block;width:100%;height:auto;overflow:visible}.grid{stroke:var(--divider-color,#e4e8eb);stroke-width:1}.hour-tick{stroke:var(--pv-muted);stroke-width:1;vector-effect:non-scaling-stroke;pointer-events:none}.axis{fill:var(--pv-muted);font-size:11px}.axis tspan{font-size:10px}.forecast-line{stroke:var(--pv-line);stroke-width:2.5;fill:none;stroke-linejoin:round}.history-line{stroke:var(--pv-archive);stroke-width:2;stroke-dasharray:5 4;fill:none}.actual-line{stroke:var(--pv-actual);stroke-width:3;fill:none;stroke-linejoin:round}.actual-partial-line{stroke-dasharray:5 4}.actual-partial-key{border-top-style:dashed}.actual-bar{fill:var(--pv-actual);fill-opacity:.25;stroke:var(--pv-actual);stroke-width:1}.now{stroke:var(--pv-muted);stroke-width:1;stroke-dasharray:2 4}.now-label{font-size:11px;fill:var(--pv-muted)}.gap{fill:var(--pv-muted);opacity:.09}.empty-plot{font-size:var(--pv-text);fill:var(--pv-muted)}.chart-note{font-size:var(--pv-text);color:var(--pv-muted);text-align:right;margin:0 0 17px;overflow-wrap:anywhere}
