...
function renderChart(state, width, selectedKey) {
  ...
  return `<svg id="interval-chart" class="chart" viewBox="0 0 ${width} 242" tabindex="0" role="group" aria-roledescription="Interaktives Diagramm" aria-labelledby="chart-title" aria-describedby="chart-description chart-help">
    <title id="chart-title">Energie je Intervall in kWh</title><desc id="chart-description">${view.historical ? "Archivierte Prognose: " + escapeHtml(view.label) : "Aktuelle Prognose durchgezogen, " + ARCHIVE_LABEL} gestrichelt, Tatsächliche Produktion als farbige Stufenlinie mit dezenten Balken.${series.raw ? " Grundmodell ohne Selbstkalibrierung strichpunktiert." : ""} Fehlende Werte bleiben leer. Alle Werte stehen auch in der Tabelle.</desc>
    <defs><clipPath id="plot-clip"><rect x="${left}" y="0" width="${width - left - 12}" height="${bottom + 2}"/></clipPath></defs>
    ${grid}<g clip-path="url(#plot-clip)">${gaps}${highlight}${actual}${paths(series.history, "history-line")}${paths(series.raw ?? [], "raw-line", "is_complete")}${paths(series.forecast, "forecast-line", "is_complete")}${paths(series.actual.map((item) => ({ ...item, energy_complete: item.energy_complete === true })), "actual-line", "energy_complete")}${now}</g><g aria-hidden="true" class="hour-ticks">${hourTicks}</g>${ticks}
